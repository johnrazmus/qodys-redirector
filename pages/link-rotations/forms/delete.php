...
if( $post_data )
{
	...
	{
		$qodys_redirector->DataType( 'link-rotation' )->DeletePosts( $post_ids );
		
		$response['results'][] = count( $post_ids ).' item'.(count( $post_ids ) > 1 ? 's' : '').' '.(count( $post_ids ) > 1 ? 'have' : 'has').' been deleted';
		
		$url = $post_data['success_url'] ? $post_data['success_url'] : $qodys_redirector->Page( 'link-rotation' )->AdminUrl();
	}
}
else
{
